...
internals.initialize = function (server, next) {
    ...
    Object.keys(collector.models).forEach((id) => {
        ...
        waterline.registerModel(modelExtended);
    });
    const config = {
        adapters: collector.adapters,
        datastores: collector.connections,
...
internals.collections = (serverFrom, realmPath) => {
    ...
    return function (all) {
        ...
        if (!waterline.ontology) {
            return {};
        }

        if (all) {
            return waterline.ontology.collections;
        }
        ...
        for (let i = 0; i < models.length; ++i) {
            collections[models[i]] = waterline.ontology.collections[models[i]];
        }
